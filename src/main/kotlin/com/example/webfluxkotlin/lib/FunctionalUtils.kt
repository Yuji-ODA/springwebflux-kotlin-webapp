...
object FunctionalUtils {
    ...
    fun <T> sequence(stream: Stream<Mono<T>>): Mono<Stream<T>> =
        stream.reduce(Mono.just(Stream.empty()),
            liftMono2 { acc, s -> Stream.concat(acc, Stream.of(s)) },
            liftMono2 { s1, s2 -> Stream.concat(s1, s2) })
    ...
    inline fun <T1, T2, R> map2(m1: Mono<T1>, m2: Mono<T2>, crossinline f: (T1, T2) -> R): Mono<R> = liftMono2(f)(m1, m2)
